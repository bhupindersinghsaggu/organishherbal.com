<?php

include('header.php');
include('function.php');

?>
<section class="page-title-section bg-img cover-background" data-overlay-dark="6" data-background="/theme/img/bg/meat-masala-bg.jpg" style="background-image: url(_/theme/img/bg/meat-masala-bg.jpg);">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
            </div>
        </div>
    </div>
</section>
<section id="moringa">
    <div class="container">
        <!-- <div class="product-heading">
            <h4 class="about-box2 border-radius-10">
                Discover the magic of spices and transform every meal
            </h4>
        </div> -->
        <div class="row align-items-center ">
            <div class="col-lg-6 mb-1-9 mb-lg-0">
                <div class="position-relative pe-lg-1-9 pe-xl-7 ps-xl-8 text-center about-style01">
                    <img src="theme/img/product/meat-masala.jpg" class="rounded z-index-1 position-relative" alt="...">
                    <h5 class="mt-3">AVAILABILITY: 50 g | 100g </h5>
                </div>
            </div>
            <div class="col-lg-6">
                <div class="">
                    <h4 class="display-20 display-sm-15 display-xl-9 mb-3" style="font-size: 35px;">Meat Masala
                    </h4>
                    <p>Our Meat Masala is a blend of strong yet complementary flavours such as Dehydrated onion, garlic, coriander, cardamom and clove, enhanced with the aromatic, delicate and versatile flavours of mace and nutmeg. Made with quality spices, this blend is a kitchen game-changer. These powders are either milled and dried or extracts which are dried resulting in a totally natural way to add meat taste to your end products. Variations such as Roast, Skin and Boiled are possible.
                    </p>
                    <div class="col-sm-12">
                        <h5>-About this item</h5>
                        <ul class="list-style2 mb-0">
                            <li> Meat masala elevates your traditional mutton dishes to give a restaurant-style taste and visual appeal.</li>
                            <li>Organish Herbal Meat Masala adds colour, flavour and aroma to simple mutton and raan dishes.</li>
                            <li>Made using Low Temperature Grinding of several Indian spices</li>

                        </ul><br>
                        <a href=<?php echo " $shoplink " ?> target=_blank class="butn secondary">Buy Now</a>
                    </div>
                </div>
            </div>
            <div class="col-lg-12 mb-2-9">
                <div class="row padding-top50">
                    <div class="col-lg-6 mb-1-9 mb-lg-0">
                        <div class="card border-0 border-radius-none">
                            <div class="card-body p-0">
                                <div class="border-bottom border-width-2 pb-3 mb-4">
                                </div>
                                <ul class="list-style5 text-center text-sm-start">
                                    <li>
                                        <div class="text-center text-sm-start d-sm-flex mb-4">
                                            <div class="">
                                                <h4 class="h5">Health Benefits</h4>
                                            </div>
                                        </div>
                                        <div class="mb-3">
                                            <div class="nutritional-info">
                                                <ul class="list-style1 mb-0">
                                                    <li>
                                                        <h4 class="h6 mb-0"><i class="ti-check-box text-secondary me-3 font-weight-500"></i>
                                                            Rich source of iron </h4>
                                                    </li>
                                                    <li>
                                                        <h4 class="h6 mb-0"><i class="ti-check-box text-secondary me-3 font-weight-500"></i>Low calories of food </h4>
                                                    </li>
                                                    <li>
                                                        <h4 class="h6 mb-0"><i class="ti-check-box text-secondary me-3 font-weight-500"></i>Good source of quality protein </h4>
                                                    </li>
                                                    <li>
                                                        <h4 class="h6 mb-0"><i class="ti-check-box text-secondary me-3 font-weight-500"></i>Low cholesterol </h4>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-6 mb-1-9 mb-lg-0">
                        <div class="card border-0 border-radius-none">
                            <div class="card-body p-0">
                                <div class="border-bottom border-width-2 pb-3 mb-4">
                                </div>
                                <ul class="list-style5 text-center text-sm-start">
                                    <li>
                                        <div class="text-center text-sm-start d-sm-flex mb-4">
                                            <div class="flex-shrink-0 mb-4 mb-md-0">
                                                <img src="theme/img/product/meat-masala-round1.jpg" class="rounded-circle" alt="...">
                                            </div>
                                            <div class="flex-grow-1 ms-sm-4">
                                                <h4 class="h5">Ingerdients</h4>
                                                <p class="mb-0 ingerdients-info">Nutrition Value Per 100g</p>
                                            </div>
                                        </div>
                                        <div class="mb-3">
                                            <div class="nutritional-info">
                                                <h4 class="h6 mb-0">Energy (Calculated) </h4>
                                                <span class="seprator"></span>
                                                <span class="text-muted font-weight-600">425 kcal</span>
                                            </div>
                                            <div class="nutritional-info">
                                                <h4 class="h6 mb-0">Total Carbohydrates</h4>
                                                <span class="seprator"></span>
                                                <span>77 g</span>
                                            </div>
                                            <div class="nutritional-info">
                                                <h4 class="h6 mb-0">Protein</h4>
                                                <span class="seprator"></span>
                                                <span>6.2 g</span>
                                            </div>
                                            <div class="nutritional-info">
                                                <h4 class="h6 mb-0">Total Fat</h4>
                                                <span class="seprator"></span>
                                                <span>10 g</span>
                                            </div>
                                            <div class="nutritional-info">
                                                <h4 class="h6 mb-0">Saturated Fatty Acid</h4>
                                                <span class="seprator"></span>
                                                <span>0.5 g</span>
                                            </div>
                                            <div class="nutritional-info">
                                                <h4 class="h6 mb-0">Total Trans Fatty Acid</h4>
                                                <span class="seprator"></span>
                                                <span>0 g</span>
                                            </div>
                                            <div class="nutritional-info">
                                                <h4 class="h6 mb-0">Cholesterol</h4>
                                                <span class="seprator"></span>
                                                <span>0 g</span>
                                            </div>
                                        </div>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<?php

include('latest-product.php');
include('footer.php');

?>